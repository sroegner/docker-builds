...
then
	POLICY="-Djava.security.manager -Djava.security.policy=/etc/accumulo/conf/accumulo.policy"
fi
...
test -z "$ACCUMULO_TSERVER_OPTS" && export ACCUMULO_TSERVER_OPTS="$TSERVER_DBG $TSERVER_OOM_OPTS $JMX_OPTS -Dcom.sun.management.jmxremote.port=26051 ${POLICY} -Xmx3072m -Xms128m "
test -z "$ACCUMULO_MASTER_OPTS"  && export ACCUMULO_MASTER_OPTS="$MASTER_DBG $MASTER_OOM_OPTS $JMX_OPTS -Dcom.sun.management.jmxremote.port=26052 ${POLICY} -Xmx3072m -Xms128m"
test -z "$ACCUMULO_MONITOR_OPTS" && export ACCUMULO_MONITOR_OPTS="$MONITOR_DBG $JMX_OPTS -Dcom.sun.management.jmxremote.port=26053 ${POLICY} -Xmx1536m -Xms64m"
test -z "$ACCUMULO_GC_OPTS"      && export ACCUMULO_GC_OPTS="$GC_DBG $JMX_OPTS -Dcom.sun.management.jmxremote.port=26054 -Xmx1536m -Xms64m"
test -z "$ACCUMULO_GENERAL_OPTS" && export ACCUMULO_GENERAL_OPTS="-XX:+UseConcMarkSweepGC -XX:CMSInitiatingOccupancyFraction=75"
test -z "$ACCUMULO_OTHER_OPTS"   && export ACCUMULO_OTHER_OPTS="-Xmx3072m -Xms64m"
export ACCUMULO_LOG_HOST=`(grep -v '^#' /etc/accumulo/conf/monitor ; echo localhost ) 2>/dev/null | head -1`
